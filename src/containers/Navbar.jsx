...
const Navbar = () => {
  ...
  return (
    <>
      <nav className=" z-40 fixed top-0 left-0 right-0 h-[75px] px-4 flex items-center justify-center border-b border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900">
        <div className="w-[1000px] flex items-center justify-between">
            ...
            {MainMenuData.map((item) => (
              <li
                className={twMerge(
                  ...
                  item.active && "bg-blue-600 text-gray-200 hover:text-gray-200"
                )}
                key={item.id}
              >
                {item.title}
          ...
          {/* Sidenav toogle */}
          <div
            className=" cursor-pointer w-[30px] h-[30px] rounded hover:bg-gray-700 flex sm:hidden items-center justify-center dark:text-gray-200"
...
const Sidenav = ({ handleClick }) => {
  return (
    <div className="z-50 fixed top-0 bottom-0 left-0 right-0 bg-black/50 backdrop-blur-sm z-50 flex">
      <div className=" flex flex-col gap-8 min-w-[300px] h-full bg-white text-gray-200 dark:bg-gray-900 slide-in-left">
        ...
        <ul className="flex flex-col gap-2 px-4">
          {MainMenuData.map((item) => (
            <li
              className={twMerge(
                " cursor-pointer rounded px-2 py-1 hover:text-blue-600 font-medium dark:text-gray-200",
                item.active && "bg-blue-600 text-gray-200 hover:text-gray-200"
              )}
              key={item.id}
            >
